...
static int test_range_unmap_small(env_t env) {
    ...
    for (int i = 0; i < NPAGE/32; i += 1) {
        seL4_Word cap_set[32];
        for (int j = 0; j < 32; j++) {
            cap_set[j] = frames[32*i + j];
        }
        seL4_ARM_VSpace_Remap_Range_t remap_ret = seL4_ARM_VSpace_Remap_Range(vspace, cap_set, 32, seL4_NoRights);
...
static int test_range_unmap_large(env_t env) {
    ...
    while (curr_addr < end) {
        ...
        test_assert(unmap_ret.remapped == 32);
    }
